...
from os.path import isfile, isdir, lexists, abspath
...
class DependencyExtractor(object):
    def __init__(self, version='2.7', verbose=False):
        super(DependencyExtractor, self).__init__()
    ...
    #     html = urllib2.urlopen('http://docs.python.org/%s/py-modindex.html' % '2.7').read()
    ...
    def exclude_project_modules(self, fp):
        contents = listdir(fp)

        if '__init__.py' in contents:   #this folder is importable, and is internal-importable
            dirname = fp[fp.rfind('/')+1:]
            self.project_modules.add(dirname)
            for content in contents:
                if isdir(fp+'/'+content):
                    self.exclude_project_modules(fp+'/'+content)    #recursively extract dependency under subdir
                elif content[-2:] == 'py':
                    self.project_modules.add(content[:-3])
        else:
            for content in contents:
                if isdir(fp+'/'+content):
                    self.exclude_project_modules(fp+'/'+content)    #recursively extract dependency under subdir
        ...
        for line in fp_opener:
            ...
            if line_tokens and (line_tokens[0] == 'import' or multiline_import):
                ...
                for comma_split_token in comma_split_tokens:
                    ...
                    if candidate and candidate not in self.builtin_modules and candidate not in self.project_modules:
...
def main():
    ...
    parser.add_argument('-v', '--verbose', action='store_true', help='show the progress')
    ...
    extractor = DependencyExtractor(verbose=args.verbose)
    if args.recursive:
        if not isdir(fp):
            log_err('[Error] "%s" is not a directory.' % fp)
            exit(1)
        else:
            extractor.exclude_project_modules(abspath(fp))
            extractor.extract_in_dir(abspath(fp))
            extractor.output()
    else:   #single file
        if isdir(fp):
    ...
    # log_err(extractor.project_modules)
